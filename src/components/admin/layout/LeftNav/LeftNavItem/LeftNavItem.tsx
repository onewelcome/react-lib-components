...
import React, { HTMLProps, MouseEvent, useRef, useState } from "react";
...
export const LeftNavItem = ({ item, navigate, previousButtonRef, closeParentList }: Props) => {
  ...
  item.active && buttonClasses.push(classes["menu-item-active"]);
      ...
      <li className={buttonWrapperClasses.join(" ")} data-testid={item.key}>
        ...
        >
          <div className={classes["menu-item-text-wrapper"]}>
            {item.iconComponent ? (
              <span className={classes["menu-item-icon"]}>
                <item.iconComponent />
              </span>
            ) : null}
            {item.path ? (
          ...
          <Icon className={classes["menu-item-expand"]} icon={Icons.ChevronDown} />
        </button>
        <ul
          id={item.key}
          ref={listRef}
