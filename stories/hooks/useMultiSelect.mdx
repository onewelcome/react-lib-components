...
In an advanced scenario (**#2**) it allows you to automatically handle `MultiSelect`'s `onAddNew` functionality, where it not only adds the new option, but it also takes care about removing such a custom option from the the list of all options when the user unpicks it.
