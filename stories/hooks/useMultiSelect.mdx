...
In an advanced scenario (**#2**) it allow you to automatically handle `MultiSelect`'s `onAddNew` functionality, where it not only adds the new option, but it also takes care about removing such a custom option from the the list of all options when the user unpicks it.
...
- `allOptions` is a string array of all options to choose from.
  - _(mandatory for scenarios #1 and #2, not mandatory for scenario #3, where the user doesn't really pick from a list of options; istead they only change the picked options)_
- `pickedOptions` is a string array of currently picked options.
  - _(mandatory)_
