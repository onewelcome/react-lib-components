...
In order to do this you can use the `useBodyClick` hook to quickly and efficiently apply an eventlistener to the body to check for this event.

## Arguments

The `useBodyClick` function accepts three arguments, the check function, callback function and the depending state variable.

### Check function

The check function is the function that will be executed everytime the user clicks on the body. It should return a boolean value to indicate wheter the callback function should be called.
...
the modal to be closed, so our check function should return false. However, if it's not the case, the user clicked outside the modal and we want to execute the callback function.
...
This function is completely up to you. In our modal example, it probably as some sort of `open` property that is set to `true` or `false` to indicate that the modal needs to be shown or not.
