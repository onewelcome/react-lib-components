...
The `useRepeater` hook is written to help with duplicating a, or a set of, component(s). This can be useful if a user can enter 1 or multiple entries of the same set of fields. If you plan to use this within a `form`, then first check if the Form Management Library you're using (`formik` or `React Final Forms`) has their own implementation of a repeater!
