...
The `useRepeater` hook is written to help with duplicating a, or a set of, component(s). This can be useful, if a user is allowed enter 1 or multiple entries of the same set of fields.
...
The repeat function does what its name suggests, create another instance of your component. You can, for example, create your own `Button` component, which has an `onClick` listener, that executes `repeat()` whenever its clicked on. This will add another
instance of the component you want to repeat inside of the `repeatedComponents` array.
...
<Canvas>
  <Story id="hooks-userepeater--repeater" />
</Canvas>
