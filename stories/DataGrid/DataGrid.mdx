...
To show filter toolbar one has to pass `filters` prop to the `DataGrid` component with `enable` property set to `true`.

User can configure the behavior of the toolbar using the `filterProps` object which has the following properties:
...
- `onFilterAdd`, `onFilterEdit`, `onFilterDelete`, `onFiltersClear` - callbacks used in the filter editor
...
To show searchbar one has to pass `search` prop to the `DataGrid` component with `enable` property set to `true`.
