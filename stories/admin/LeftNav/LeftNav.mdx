...
The `<LeftNav />` component is a vertical navigation menu designed for sidebars. It supports nested menu items, disabled elements, and custom icons.
...
The same applies to the `disabled` flag. If the parent element has it set, all its children will be disabled as well.

The `<LeftNav />` requires `react-router`. The edge links are rendered using `react-router`'s `<Link />` component.

The `<LeftNav />` calls two callback methods on different conditions:

- `navigate(path: string)` - Called directly when the user presses a parent element (button) to align with the behavior of `react-router`'s `<Link />` component.
- `onItemClick(path: string, button: boolean)` - Called when pressing parent elements (button) or edge links (link).
...
An example with disabled items:

<Canvas of={LeftNavStories.DisabledItemsLeftNav} />
