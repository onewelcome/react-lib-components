...
This also means that not all native `select` HTML attributes are available. However, we support `multiple` attribute in [`MultiSelect` component](?path=/docs/components-inputs-select-wrapper--docs) component.
