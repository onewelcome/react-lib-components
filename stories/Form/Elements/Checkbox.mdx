...
`Checkbox` is a `Form` component and should be nested in one **or** it should have a `form` attribute with the `id` property set to the id of the `form` you're linking it to.
...
<Canvas>
  <Story id="stories-form-elements-checkbox--single-checkbox" />
</Canvas>
...
<Canvas>
  <Story id="stories-form-elements-checkbox--nested-checkbox" />
</Canvas>
...
There are multiple ways to solve this. In this example we chose for 5 separate state variables, but you can also choose to use `useReducer` to manage all of the state variables. Alternatively you could also use a single `useState` with an object that manages all the state.
