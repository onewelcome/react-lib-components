...
The `Checkbox` component is a native HTML `input` element. And just like the native elements, it should be linked to a `form`. This can be done by either nesting the `Checkbox` component inside of a `Form` component, or by linking to a `form` by using the appropriate `form` attribute.
