...
This example uses `useMultiSelect` which automates handling of `onChange` and `onAddNew` (for more examples [see docs for MultiSelectWrapper](?path=/docs/components-inputs-multiselect-wrapper--docs)).
...
<Canvas of={MultiSelectStories.MultiSelectUseBasic} />
